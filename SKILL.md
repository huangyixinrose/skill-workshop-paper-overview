...
description: >
  ...
  报告包含：编者整体判断、分类总表、逐篇速查卡、主线综述、少量关键架构图。
...
这条流水线读完一个 workshop 的全部论文，输出单文件 HTML 综述。成品包含四个板块：编者整体判断（卷首）、分类总表、逐篇速查卡（含少量架构图）、按主线展开的综述。

设计原则：**能用脚本做的不交给模型**。整条流水线只在 4 处调用 LLM——侦察主页提取论文清单、精读每篇论文、撰写主线综述与查漏、编者写卷首判断——其余环节（下载、抽文本、分组、抠图、渲染 HTML）全由脚本完成。
...
| 4. 并行精读 | 每篇一个子代理结构化精读 | `txt/*.txt` + 类目 | `data/digests.json` | `workflow/review_workshop.template.js` | 便宜模型 |
| 5. 主题综述 + 查漏 | 按主线写综述、critic 检查 | digests + 类目 | `data/sections.json`、`data/critic.txt` | `prompts/theme_writer.md`、`prompts/critic.md` | 便宜模型 |
| 6. 配图 | 为少数论文裁剪关键架构图 | PDF + digests | `figures/fig_<id>.png` | `scripts/extract_figure.py` | 强模型（看图定框） |
| 7. 卷首 + 组装 | 编者写整体判断、渲染 HTML | sections + critic + digests | `report.html` | `prompts/preface.md`、`scripts/build_report.py` | 强模型（卷首）/ 无（组装） |
...
链接补全是这一步的主要坑（各家主页格式差异大），`prompts/recon.md` 里列出了 CVF / arXiv / OpenReview / 站内相对链接等常见规则。无全文或撤稿的论文不进 manifest，放入 `data/missing.json`，后续由 recovery 子代理检索。
...
每篇论文分配一个子代理（便宜模型），读取 `txt/<id>.txt`，按 `prompts/reader.md` 的提示词精读，强制输出符合 `schemas/digest.schema.json` 的结构化数据。schema 不合格时自动重试。

同时启动 1 个 recovery 子代理，用 WebSearch 检索 `data/missing.json` 中缺失论文的预印本。

产出汇总为 `data/digests.json`。recovery 结果写入 `data/recovery.txt`。

digest schema 的核心字段：`id`、`title`、`team`、`track`、`category`、`problem`、`method`、`key_results`、`limitations`、`relevance`、`card_zh`（120-180 字速查卡）、`figure_essential`（是否值得配图）、`figure_ref`、`figure_caption`。完整定义见 `schemas/digest.schema.json`。

**为什么强制结构化输出 + 重试：** 多个子代理并行跑，结果必须能拼起来。没有 schema 约束，输出格式漂移会导致后续组装失败。
...
**5a. 主线写手。** 按 `data/cats.json` 的类目分组（确定性脚本），每条主线分配一个写手子代理（便宜模型），按 `prompts/theme_writer.md` 撰写 600-1000 字综述。产出写入 `data/sections.json`，顺序与 `cats.json` 对齐。

**5b. 查漏 critic。** 跑一个 critic 子代理（便宜模型），按 `prompts/critic.md` 审查全部 digests + sections 草稿，检查：归错类或遗漏的论文、缺论文支撑的结论、不可比数字并排、绝对值写成相对提升、以偏概全的断言、缺失的跨主线视角。
...
**7a. 卷首（强模型亲自写）。** 这是整份综述最高价值的部分，不外包给子代理。通读 `data/sections.json` + `data/critic.txt`，按 `prompts/preface.md` 写出编者"整体判断"——核心成果、能力边界、未来趋势，每条判断都挂具体证据（哪篇、什么数字）。优先写 critic 发现的跨主线主轴。产出 `data/preface.html`（HTML 片段）。
...
| 角色 | 模型档位 | 说明 |
|------|----------|------|
| 精读子代理（每篇） | 便宜（Sonnet/Haiku） | 并发量大，是 token 消耗大头 |
| recovery 子代理 | 便宜（Sonnet） | 检索缺失论文 |
...
5. **配图少而精。** 只配真帮理解的，不因"每篇都有架构图"就每篇配。（阶段 6）
6. **断点续跑。** Workflow 支持 `resumeFromRunId`，跑到一半挂了能续。（阶段 4）
7. **侦察是最脆弱的环节。** 各家主页格式差异大，解析不了就退回让用户贴链接清单。（阶段 1）
