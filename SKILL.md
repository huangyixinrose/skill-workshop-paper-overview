...
**7a. 卷首（强模型亲自写）。** 这是整份综述最高价值的部分，不外包给子agent。通读 `data/sections.json` + `data/critic.txt`，按 `prompts/preface.md` 写出编者"整体判断"——核心成果、能力边界、未来趋势，每条判断都挂具体证据（哪篇、什么数字）。优先写 critic 发现的跨主线主轴。产出 `data/preface.html`（HTML 片段）。
